...
def detect_orientation(body, canal_points):
  ...
  point_a_i = closest_i - 3
  if point_a_i < 0:
    point_a_i = 0
  point_b_i = closest_i + 3
  if point_b_i >= len(canal_points):
      point_b_i = len(canal_points) - 1

  point_a = canal_points[point_a_i]
  point_b = canal_points[point_b_i]

  return (body_centroid, point_a, point_b)


def find_plane(centroid, upper, lower, name):
  markup = slicer.build_markups([slicer.build_markup_fiducial([upper, lower, centroid], "k")])
  with open(f"./target/{name}/key_points.json", "w") as f:
    json.dump(markup, f)
  plane_anchor = (np.array(upper) + np.array(lower)) * 0.5

  y_vector = np.array(upper) - np.array(lower)
  y_vector = y_vector / np.linalg.norm(y_vector)
  if y_vector[2] > 0:
    y_vector = -y_vector

  oc_vector = np.array(centroid) - plane_anchor
  oc_vector = oc_vector / np.linalg.norm(oc_vector)

  z_vector = np.cross(oc_vector, y_vector)

  x_vector = np.cross(y_vector, z_vector)

  return plane_anchor, x_vector, y_vector, z_vector
...
def find_measurement_points(image, point_center, point_a, point_b, name):
  os.makedirs(f"./target/{name}", exist_ok=True)
  plane_anchor, x_vector, y_vector, z_vector = find_plane(point_center, point_a, point_b, name)
...
def prepare_spinal_canal_points():
  ...
  closed_canal = sitk.BinaryMorphologicalClosing(canal, kernelRadius=(2, 2, 2))
  ...
  return approx(canal_points)


os.makedirs("./target", exist_ok=True)
image = sitk.ReadImage("../data/case22/seg.nii.gz")
...
for (key1, value1), (key2, value2) in zip(zip(keys, values), zip(keys[1:], values[1:])):
  upper = image == value1
  lower = image == value2
  upper = postprocess_label_image(upper)
  lower = postprocess_label_image(lower)
  upper_centroid, upper_a, upper_b = detect_orientation(upper, canal_points)
  lower_centroid, lower_a, lower_b = detect_orientation(lower, canal_points)

  ((up_top_start, up_top_end, up_bot_start, up_bot_end)) = vertebrae_points[
    key1] if key1 in vertebrae_points.keys() else find_measurement_points(
    upper, upper_centroid, upper_a, upper_b, key1)

  ((low_top_start, low_top_end, low_bot_start, low_bot_end)) = vertebrae_points[
    key2] if key2 in vertebrae_points.keys() else find_measurement_points(
    lower, lower_centroid, lower_a, lower_b, key2)
