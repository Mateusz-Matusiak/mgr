...
Skolioza wrodzona jest spowodowana nieprawidłowym rozwojem kręgosłupa w stadium embrionalnym, co może obejmować nieprawidłowy kształt lub nieprawidłową liczbę kręgów.
...
Istotne jest wczesne wykrycie i ocena stopnia zaawansowania choroby.
...
\begin{itemize}
    ...
    \item \textbf{Ekstrakcja przekrojów 2D} -- algorytm dokonuje próbkowania (resampling) wolumenu do lokalnej płaszczyzny czołowej każdego kręgu.
...
W ten sposób obszar kanału kręgowego staje się bardziej odporny na luki w segmentacji, co zwiększa obszar dostępny do dalszego przetwarzania.
...
Następnie dla otrzymanego szkieletu wyznaczany jest zorientowany prostopadłościan opisujący (ang. \textit{oriented bounding box}) przy użyciu filtru \textit{LabelShapeStatisticsImageFilter}, a szkielet przepróbkowywany jest do układu współrzędnych tego prostopadłościanu.
Zabieg ten sprawia, że oś podłużna nowej siatki obrazu podąża za rzeczywistym przebiegiem kanału kręgowego, a nie za osią skanera, dzięki czemu kolejne warstwy poprzeczne pozostają w przybliżeniu prostopadłe do kanału niezależnie od ułożenia pacjenta.
Przyjęto rozdzielczość przepróbkowania równą $1{\times}1{\times}2$ mm, przy czym trzecia składowa odpowiada krokowi wzdłuż osi podłużnej kanału.
Kolejnym krokiem jest przejście przez wszystkie warstwy poprzeczne przepróbkowanego szkieletu i wyznaczenie w każdej z nich centroidu, który stanowi punkt linii środkowej kanału kręgowego.
...
\begin{itemize}
    \item $h_{step}$ –- referencyjny krok wzdłuż osi podłużnej, odpowiadający typowej grubości warstwy badania MRI w analizowanym zbiorze danych (w implementacji przyjęto $h_{step} = 3{,}2$ mm).
    \item $T_{thresh}$ –- próg detekcji ubytku, definiujący minimalną odległość uznawaną za lukę w danych (w implementacji $T_{thresh} = h_{step} = 3{,}2$ mm).
\end{itemize}

Wartość $h_{step}$ jest większa od kroku przepróbkowania szkieletu, wynoszącego 2 mm, dzięki czemu naturalne wahania położenia centroidów w sąsiednich warstwach nie są interpretowane jako ubytki segmentacji -- za lukę uznawana jest dopiero przerwa odpowiadająca ponad półtorej warstwie przepróbkowanego szkieletu.
Należy przy tym zaznaczyć, że odległość $\Delta z_i$ mierzona jest wzdłuż osi pionowej układu współrzędnych pacjenta, a nie wzdłuż osi podłużnej kanału kręgowego.
Dla odcinków silnie odchylonych od pionu wyznaczona różnica jest zatem mniejsza od rzeczywistej odległości między punktami, co czyni przyjęte kryterium zachowawczym: algorytm może pominąć niewielki ubytek, ale nie zgłosi ubytku nieistniejącego.
...
Płaszczyzna przekroju rozpięta jest na osiach $\vec{x}$ (boczno-przyśrodkowej) oraz $\vec{y}$ (pionowej), co odpowiada lokalnej płaszczyźnie czołowej kręgu.
Wybór ten wynika bezpośrednio z definicji metody Cobba, która opisuje deformację w płaszczyźnie czołowej; oś $\vec{z}$ pełni w tym przypadku rolę normalnej płaszczyzny pomiarowej.
...
\label{fig:scoliosis-left-plane}
    ...
    \end{subfigure}
    \caption{Wizualizacja trójwymiarowa pojedynczego kręgu lędźwiowego wraz z nałożoną lokalną płaszczyzną przekroju czołowego, wyznaczoną na podstawie bazy ortonormalnej $\{\vec{x}, \vec{y}, \vec{z}\}$}
\end{figure}
...
\begin{figure}[H]
    ...
    \captionsetup{justification=centering, font=small}
    \caption{Wizualizacja dwuwymiarowa pojedynczego kręgu lędźwiowego na lokalnej płaszczyźnie przekroju czołowego.}
...
W pierwszej kolejności przekrój poddawany jest operacji wypełnienia otworów wewnętrznych (\texttt{sitk.BinaryFillhole}).
Usuwa ona niezasegmentowane obszary znajdujące się wewnątrz trzonu, które w przeciwnym razie zostałyby wykryte przez detektor krawędzi jako dodatkowe kontury wewnętrzne.

Następnie, w celu wyeliminowania drobnych artefaktów powstałych na etapie segmentacji, obraz poddawany jest operacji morfologicznego otwarcia (ang. \textit{morphological opening}, z biblioteki \texttt{cv2.morphologyEx(image, cv2.MORPH\_OPEN, np.ones((5, 5), np.uint8), iterations=2)})\cite{morph-opening}.
...
\begin{figure}[H]
    ...
    \captionsetup{justification=centering, font=small}
    \caption{Wizualizacja wyznaczonej krawędzi pojedynczego kręgu lędźwiowego na lokalnej płaszczyźnie przekroju czołowego.}
    \label{fig:edge}
\end{figure}
...
\begin{equation} \label{eq:spine_vec}
    \vec{s} = \bar{p}_{upper} - \bar{p}_{lower}
\end{equation}

Pierwszy wektor bazy płaszczyzny wyznaczany jest jako iloczyn wektorowy kierunku przebiegu kręgosłupa oraz wektora normalnego, dzięki czemu leży on w płaszczyźnie pomiarowej i jest prostopadły do osi podłużnej analizowanego odcinka:

\begin{equation} \label{eq:plane_x}
    \vec{x}_{plane} = \frac{\vec{s} \times \vec{n}}{|\vec{s} \times \vec{n}|}
\end{equation}

Drugi wektor bazy uzupełnia układ do bazy ortonormalnej rozpinającej płaszczyznę:

\begin{equation} \label{eq:plane_y}
    \vec{y}_{plane} = \frac{\vec{n} \times \vec{x}_{plane}}{|\vec{n} \times \vec{x}_{plane}|}
\end{equation}

Tak zdefiniowana baza $\{\vec{x}_{plane},\, \vec{y}_{plane}\}$ pozwala wyrazić położenie zrzutowanych punktów narożnych w dwuwymiarowym układzie współrzędnych wspólnej płaszczyzny pomiarowej.
...
Analizą objęto kręgi od Th11 do L5.
Kręg S1, mimo że jest obecny w mapie segmentacji, został wyłączony z pomiaru po konsultacji z lekarzem radiologiem.
Stanowi on element kości krzyżowej, a jego górna powierzchnia nie jest płytką graniczną porównywalną z płytkami kręgów lędźwiowych: jej nachylenie wynika z ustawienia miednicy, a nie z ewentualnego skrzywienia odcinka lędźwiowego.
Uwzględnienie tego kręgu w analizie prowadziło do zawyżania wyznaczanego kąta w parach obejmujących poziom L5/S1.

Obliczenia kąta Cobba wykonywane są dla wszystkich par kręgów $(k_i,\, k_j)$ spełniających warunek $j \geq i + 2$, co wyklucza pary kręgów bezpośrednio sąsiadujących.
Dla takich par wyznaczony kąt zdominowany jest przez klinowatość krążka międzykręgowego oraz lokalną asymetrię płytek granicznych, a nie przez rzeczywiste skrzywienie kręgosłupa, przez co wynik nie odpowiadałby pomiarowi wykonywanemu metodą Cobba. Wynikowy kąt maksymalny wyznaczany jest jako:

\begin{equation} \label{eq:max_cobb}
    \alpha_{max} = \max_{\substack{i,j \\ j \geq i+2}} |\alpha(k_i, k_j)|
\end{equation}

Wynikiem działania algorytmu jest wartość kąta Cobba wyrażona w stopniach, para kręgów wyznaczająca największą deformację oraz strona skrzywienia (prawostronna lub lewostronna).
Klasyfikacja badania jako zdrowego lub patologicznego polega na porównaniu wyznaczonej wartości $\alpha_{max}$ z przyjętą wartością progową.
Punktem odniesienia jest kryterium Scoliosis Research Society, zgodnie z którym skrzywienie o kącie Cobba przekraczającym 10\degree{} stanowi podstawę rozpoznania skoliozy.
Kryterium to sformułowano jednak dla pomiaru wykonywanego na radiogramie w pozycji stojącej, podczas gdy analizowane segmentacje pochodzą z badań MRI wykonywanych w pozycji leżącej, w której skrzywienie bywa mniej nasilone.
Z tego powodu wartość progowa stosowana przez opisywany algorytm została dobrana empirycznie na podstawie zbioru badań referencyjnych; procedurę jej wyznaczenia oraz uzyskane wyniki przedstawiono w dalszej części pracy. Dane pośrednie - punkty kanału kręgowego, wektory lokalnych układów współrzędnych oraz punkty narożne trzonów - są eksportowane do plików JSON kompatybilnych z oprogramowaniem 3D Slicer, co umożliwia wizualną weryfikację wyników przez lekarza.
\subsection{Złożoność obliczeniowa}
\label{subsec:zlozonosc-skolioza}

Niech $V$ oznacza liczbę wokseli wolumenu wejściowego, $N$ -- liczbę punktów wyznaczonej linii środkowej kanału kręgowego, a $K$ -- liczbę kręgów obecnych w segmentacji.
W analizowanym odcinku $K \leq 7$, a typowa wartość $N$ mieści się w rzędzie setek punktów, podczas gdy $V$ osiąga rząd milionów wokseli.

Etap wstępnego przetwarzania kanału kręgowego zdominowany jest przez operacje wolumetryczne.
Scalenie etykiet oraz szkieletyzacja metodą Lee wymagają przejrzenia całego wolumenu, ich koszt jest zatem liniowy względem liczby wokseli i wynosi $O(V)$.
Przepróbkowanie szkieletu do układu zorientowanego prostopadłościanu opisującego oraz wyznaczenie centroidów w kolejnych warstwach poprzecznych mają tę samą złożoność.
Rekonstrukcja ubytków interpolacją funkcjami sklejanymi oraz późniejsza reparametryzacja krzywej według skumulowanej długości łuku wymagają rozwiązania układu równań o strukturze trójdiagonalnej, co realizowane jest w czasie $O(N)$.

Analiza pojedynczego kręgu obejmuje wyznaczenie centroidu trzonu, odszukanie najbliższego punktu linii środkowej, wyznaczenie lokalnej bazy ortonormalnej oraz ekstrakcję przekroju wraz z detekcją punktów narożnych.
Wyszukiwanie najbliższego punktu realizowane jest metodą przeglądu zupełnego i ma koszt $O(N)$.
Analiza głównych składowych wykonywana jest na oknie o stałym rozmiarze 21 punktów, a przepróbkowanie przekroju oraz operacje wizji komputerowej -- otwarcie morfologiczne, detekcja krawędzi metodą Canny'ego, wyznaczenie otoczki wypukłej i aproksymacja wielokątowa -- działają na obrazie o stałym rozmiarze $250 \times 250$ pikseli.
Koszt tych operacji nie zależy zatem od rozmiaru danych wejściowych, a łączna złożoność etapu analizy kręgów wynosi $O(K \cdot N)$.

Ostatni etap, obejmujący wyznaczenie wspólnej płaszczyzny pomiarowej i obliczenie kąta, wykonywany jest dla każdej pary kręgów spełniającej warunek $j \geq i+2$, czyli dla $\frac{(K-1)(K-2)}{2}$ par.
Dla każdej pary dekomponowana jest metodą SVD macierz o stałym rozmiarze $8 \times 3$, zawierająca punkty narożne obu kręgów, a następnie wykonywane są rzutowania i obliczenia trygonometryczne na stałej liczbie wektorów.
Koszt pojedynczej pary jest więc stały, a złożoność całego etapu wynosi $O(K^2)$.
Przy $K \leq 7$ odpowiada to co najwyżej piętnastu analizowanym parom, wobec czego składnik kwadratowy pozostaje w praktyce pomijalny.

Sumaryczna złożoność algorytmu wynosi:

\begin{equation} \label{eq:zlozonosc-skolioza}
    O(V + K \cdot N + K^2)
\end{equation}

i pozostaje zdominowana przez składnik $O(V)$, odpowiadający operacjom wolumetrycznym wykonywanym na pełnym obrazie wejściowym.
Oznacza to, że czas przetwarzania zależy przede wszystkim od rozdzielczości badania, a nie od liczby analizowanych kręgów ani od stopnia skrzywienia kręgosłupa.
Warto zaznaczyć, że analiza poszczególnych kręgów przebiega niezależnie, co umożliwia zrównoleglenie tego etapu, jeśli czas przetwarzania okazałby się istotnym ograniczeniem we wdrożeniu klinicznym.
