...
Radiogram kręgosłupa stanowi podstawowe narzędzie obrazowe wykorzystywane w ocenie skoliozy, a szczególnie w ilościowej analizie deformacji czołowie metodą Cobba.
Standardem jest wykonanie projekcji PA (tylnio-przedniej) całego kręgosłupa w pozycji stojącej, co umożliwia uwzględnienie obciążenia grawitacyjnego i realistyczną ocenę stopnia skrzywienia.
...
Mimo że badanie RTG jest dwuwymiarowe i niesie ograniczenia w ocenie rotacji osiowej, pozostaje metodą referencyjną w diagnostyce skoliozy ze względu na wysoką dostępność, powtarzalnośc oraz możliwość precyzyjnego wyznaczenia kąta skrzywienia.
...
Pomiar powinien być wykonywany z zachowaniem jednolitej techniki, ponieważ metoda Cobba wykazuje pewien poziom zmienności obserwatora.
...
Kąt poniżej 10\degree{} traktuje się jako asymetrię tułowia, natomiast wartości równe lub przekraczające 10\degree{} stanowi kryterium rozpoznania skoliozy.
...
Deformacje przekraczające 45--50\degree{} rozważa się w kontekście leczenia operacyjnego, zwłaszcza w przypadku progresji.
Metoda Cobba odgrywa również istotną rolę w ocenie dynamiki skrzywienia -- progresja o więcej niż 5\degree{} pomiędzy kolejnymi badaniami RTG uznawana jest za klinicznie istotną.
...
Metoda zaczyna się od połączenia trzech części segmentacji odpowiadającym kanałowi kręgowemu, kanału kręgowego, końskiego ogona  oraz worka oponowego, w jedną.
W ten sposób obszar kanału kręgowego jest bardziej odporny na `przerwy` w segmentacji, zwiększa obszar do późniejszego procesowania.

Następnym krokiem jest przekazanie złączonych etykiet do funkcji  \texttt{skeletonize} z argumentem \texttt{method=lee} z pakietu \texttt{scikit-image} w celu wyznaczenie szkieletu kanału kręgowego\cite{scikit-skeletonize}.
Następnie za zwróconym zestawie pikseli (szkielecie) dokonano przepróbkowania (resamplingu) do przestrzeni wejściowego kanału kręgowego.
